...
### [2026-02-24] - 精確手術級修復
- **核心優化**：修復了因過度攔截 Google/YouTube 核心網域導致的 `ERR_BLOCKED_BY_CLIENT` 錯誤。
- **規則調整**：
  - 在 `security.json` 的 `http_block` (ID 9001) 規則中加入 Google 排除清單。
  - 在 `iframe_block` (ID 9003) 規則中加入 YouTube/Facebook/Apple 排除清單，解決第三方登入與影片加載問題。
  - 移除 `ads.json` 中對 `googlesyndication.com` 與 `adservice.google.com` 的激進封鎖，確保 Google 搜尋功能秒開。
- **目標**：實現「最強防禦」與「最高穩定性」的 CEO 級別平衡。
